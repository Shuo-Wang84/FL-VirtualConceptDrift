...
def driftDetection(window_data):
    """使用LDDDSDA方法"""
    lddd = LDDDSDA(batch_size=500, train_size=500, rho=0.8, alpha=0.8)
    d_points = []
    d_points_y = []
    ...
    # lddd.add_element(d_points, d_points_y)
    # if lddd.in_concept_change:
    #     return True

    # 逐行提取并输出
    for row, y in zip(feature_data, feature_data_y):
        lddd.add_element(row, y)
        if lddd.in_concept_change:
            return True
    return False
...
def federated_stream_artificial():
    ...
    # plt.grid(True)
    # plt.savefig(f"./new-results/global_LDDDSDA_ED_{test_name}_acc.png")
    ...
    # 保存漂移点图
    plt.savefig(f"./new-results/{id}_idddsda_alp{alpha}_{drift_Flie_name}.png")
    plt.show()
    plt.pause(0.1)
...
if __name__ == "__main__":
    ...
    # test_name = f"vitual_sudden_0.csv_g{global_epochs}_l{local_epoch}"
