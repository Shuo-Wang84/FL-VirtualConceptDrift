...
def driftDetection(window_data):
    # """使用ttest_ind方法分别测试ED""" #只在几个不同点检测到
    # d_points = []
    # p = 0
    ...
    #
    #     # print(f"mean1={batch1_mean},mean2={batch2_mean}")
    #     # 使用 t-检验检测均值是否发生显著变化
    #     t_stat, p_value = ttest_ind(batch1, batch2, equal_var=False)
    #     p += p_value
    #
    # if p < alpha:
    ...
    # 使用 t-检验检测均值是否发生显著变化
    ks_statistic, p_value = ttest_ind(scalars_np_1, scalars_np_2)
...
def federated_stream_artificial():
    ...
    window_data_list = [FixedSizeWindow(28) for _ in range(num_clients)]
    ...
    for epoch in range(global_epochs):
        total_accuracy = 0.0
        local_models = []
        if_detection = [False] * num_clients
    ...
    # plt.grid(True)
    # # plt.savefig(f"./new-results/global_ttest_ED_alp{alpha}_{test_name}_acc.png")
    ...
    # 保存漂移点图
    plt.savefig(f"./new-results/{id}_ttest_alp{alpha}_{drift_Flie_name}.png")
    plt.show()
    plt.pause(0.1)
