...
from libcdd.unsupervised_drift.ld3_ import LD3
...
def driftDetection(window_data):
    """使用LD3方法测试数据"""
    # 创建 LD3 实例
    ld3_detector = LD3(k=1, window_size=60, detection_window_size=30)
    d_points = []
    d_points_y = []
    ...
    # 逐行提取并输出
    for row, y in zip(d_points, d_points_y):
        ld3_detector.add_element(row)
        # 检测是否发生漂移
        drift_detected = ld3_detector.detected_change()
...
def federated_stream_artificial():
    ...
    # plt.grid(True)
    # plt.savefig(f"./new-results/global_LD3_alp{alpha}_{test_name}_acc.png")
    ...
    # 保存漂移点图
    plt.savefig(f"./new-results/{id}_LD3_alp{alpha}_{drift_Flie_name}.png")
    plt.show()
    plt.pause(0.1)
