...
from libcdd.unsupervised_drift.IKS import IKS
...
def driftDetection(window_data, ca):
    """使用IKS-bdd方法测试数据总的"""
    iks_detector = IKS()
    d_points = []
    ...
    k = len(feature_data2)
    # 逐行提取并输出
    for point in zip(feature_data2):
        i = i + 1
        if i <= k / 2:
            iks_detector.Add(point, 0)
        else:
            iks_detector.Add(point, 1)

    result = iks_detector.Test(ca=ca)
    if result:
        return True
    return False
    ...
    #     batch2 = feature_data[n][int(k / 2): int(k)]
    # for row1, row2 in zip(batch1, batch2):
    #     iks_detector.Add(row1, 0)
    #     iks_detector.Add(row2, 1)
    #
    # result = iks_detector.Test(ca=ca)
    # if result:
...
def federated_stream_artificial():
    ...
    for epoch in range(global_epochs):
        ...
        for client_id in range(num_clients):
            ...
            # 检测漂移
            ifdetection = driftDetection(window_data_list[client_id], ca)
            if ifdetection:
    ...
    # plt.grid(True)
    # # plt.savefig(f"./new-results/global_IKS-bdd_ED_ca{ca}_{test_name}_acc.png")
    ...
    # 保存漂移点图
    plt.savefig(f"./new-results/{id}_IKSbdd_{drift_Flie_name}.png")
    plt.show()
    plt.pause(0.1)
