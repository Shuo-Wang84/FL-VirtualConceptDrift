...
def driftDetection(window_data):
    # """使用LSDDINC方法分别测试数据每个维度,使用前一段和后一段的数据"""
    # cdt_detector = LSDDINC(train_size=200, window1_size=100, window2_size=100, u=0.0001, bootstrap_num=2000)
    # d_points = []
    ...
    #     batch1 = feature_data[n][:int(k)]
    #     # 使用 ks_2samp检测均值是否发生显著变化
    #     cdt_detector.add_element(batch1)
    #     if cdt_detector.in_concept_change:
    ...
    """使用LSDDINC方法测试数据总的"""
    inc_detector = LSDDINC(train_size=200, window1_size=100, window2_size=100, u=0.01, bootstrap_num=2000)
    d_points = []
    for d_point in window_data.get_window_data():
    ...
    for row in feature_data:
        # print(f"row={row}")
        inc_detector.add_element(row)
        if inc_detector.in_concept_change:
            return True
    return False
...
def federated_stream_artificial():
    ...
    plt.grid(True)
    plt.savefig(f"./new-results/global_LSDDINC_ED_alp{0.005}_{test_name}_acc.png")
    ...
    # 保存漂移点图
    plt.savefig(f"./new-results/driftp_LSDDINC_ED_alp{0.005}_{test_name}.png")
    plt.show()
    plt.pause(0.1)
...
if __name__ == "__main__":
    ...
    test_name = f"vitual_sudden_MD_g{global_epochs}_l{local_epoch}"
    drift_Flie_name = 'vitual_sudden_MD'
