...
from libcdd.unsupervised_drift.OCDD import *
import libcdd.unsupervised_drift.OCDD
...
def driftDetection(window_data):
    ...
    df_labels = pd.DataFrame(feature_data_y, columns=['label'])
    # 合并两个DataFrame
    result_df = pd.concat([df_features, df_labels], axis=1)
    # 将标签列转换为整数类型
    ...
    result_df[['feature1', 'feature2']] = scaler.fit_transform(result_df[['feature1', 'feature2']])
    # print("result_df", result_df)
    # 运行 OCDD 算法
    final_acc, st_rec = unsupervised_analysis(result_df, nu, size, percent)
    # 判断是否检测到漂移
    drift_detected = st_rec[-1] == 1  # st_rec 中的最后一个元素为 1 表示漂移检测到了
    if drift_detected:
        return True
    return False
...
def federated_stream_artificial():
    ...
    # plt.grid(True)
    # plt.savefig(f"./new-results/global_OCDD_nu{nu}_size{size}_percent{percent}_{test_name}.png")
    ...
    # 保存漂移点图
    plt.savefig(f"./new-results/{id}_OCDD_{drift_Flie_name}.png")
    plt.show()
    plt.pause(0.1)
...
if __name__ == "__main__":
    ...
    padding = 50
    # 设置显著性水平
    nu = 0.999  # One-Class SVM 中的 nu 参数 越小越敏感
    size = 99  # 数据窗口的大小 越小越敏感
    percent = 0.999  # 漂移检测的阈值百分比 越小越敏感
