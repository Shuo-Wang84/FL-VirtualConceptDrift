...
from scipy.stats import ks_2samp, kurtosis, skew, kruskal,mannwhitneyu
...
def driftDetection(window_data):
    ...
    #     ks_statistic, p_value = stats.mannwhitneyu(batch1, batch2)
    #     p += p_value
    #
    # if p < alpha:
    ...
    # 使用 t-检验检测均值是否发生显著变化
    ks_statistic, p_value = stats.mannwhitneyu(scalars_np_1, scalars_np_2) #stats.kruskal(x, y)
...
def federated_stream_artificial():
    ...
    window_data_list = [FixedSizeWindow(10) for _ in range(num_clients)]
    ...
    for epoch in range(global_epochs):
        ...
        for client_id in range(num_clients):
            ...
            # 转变window_data_list[client_id]
            data_in_window_0 = window_data_list[client_id].get_window_data()
            # print(f"data_in_window_0={data_in_window_0}")
            # 初始化空列表来存储合并后的数据流
            combined_data = [[], []]
            for data_stream in data_in_window_0:
                # 循环遍历每个数据流并连接它们
                for i in range(len(data_stream)):  # 两个张量
                    combined_data[i].append(data_stream[i])
            # 使用torch.cat将所有数据流连接在一起
            result_data = [torch.cat(combined_data[0], dim=0), torch.cat(combined_data[1], dim=0)]
    ...
    plt.grid(True)
    #plt.savefig(f"./new-results/global_mannwhitneyu_ravel_alp{alpha}_{test_name}_acc.png")
    ...
    # 保存漂移点图
    plt.savefig(f"./new-results/{id}_mannwhitneyu_alp{alpha}_{drift_Flie_name}.png")
    plt.show()
    plt.pause(0.1)
