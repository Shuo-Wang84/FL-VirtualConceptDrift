...
from scipy.stats import ks_2samp, kurtosis, skew, kruskal
...
def driftDetection(window_data):
    ...
    #     # 使用 kruskal检测均值是否发生显著变化
    #     ks_statistic, p_value = stats.kruskal(batch1, batch2)
    #     p += p_value
    #
    # if p < alpha:
    ...
    """使用stats.kruskal(x, y)方法测试数据总的，每一维都放进去"""
    d_points = []
    ...
    # 使用 t-检验检测均值是否发生显著变化
    ks_statistic, p_value = stats.kruskal(scalars_np_1, scalars_np_2) #stats.kruskal(x, y)
...
def federated_stream_artificial():
    ...
    window_data_list = [FixedSizeWindow(14) for _ in range(num_clients)]
    ...
    # plt.grid(True)
    # # plt.savefig(f"./new-results/global_Kruskal_ED_alp{alpha}_{test_name}_acc.png")
    ...
    # 保存漂移点图
    plt.savefig(f"./new-results/{id}_kruskal_alp{alpha}_{drift_Flie_name}.png")
    plt.show()
    plt.pause(0.1)
