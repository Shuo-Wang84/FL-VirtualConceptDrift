...
def driftDetection(window_data):
    """使用ISDD_CDT方法"""
    cdt_detector = LSDDCDT(train_size=200, window_size=100, u_s=0.02, u_w=0.01, u_c=0.001, bootstrap_num=2000)
    d_points = []
...
def federated_stream_artificial():
    ...
    plt.grid(True)
    plt.savefig(f"./new-results/global_ISDD_CDT_ED_alp{0.005}_{test_name}_acc.png")
    ...
    # 保存漂移点图
    plt.savefig(f"./new-results/driftp_ISDD_CDT_ED_alp{0.005}_{test_name}.png")
    plt.show()
    plt.pause(0.1)
...
if __name__ == "__main__":
    ...
    print(device)
    model = MLP(input_size=2, hidden_size=512, output_size=2)
    model = model.to(device)
    test_name = f"vitual_sudden_g{global_epochs}_l{local_epoch}"
    drift_Flie_name = 'vitual_sudden'
