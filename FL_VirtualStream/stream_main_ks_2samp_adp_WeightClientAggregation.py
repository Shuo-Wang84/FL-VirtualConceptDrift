...
#         super(NewModel, self).__init__()
#         self.fc1 = nn.Linear(input_size, hidden_size1)  # 第一个全连接层，8个输入，64个输出
#         self.relu = nn.ReLU()
#         self.fc2 = nn.Linear(hidden_size1, hidden_size2)  # 第二个全连接层，64个输入，32个输出
#         self.relu = nn.ReLU()
#         self.fc3 = nn.Linear(hidden_size2, output_size)   # 输出层，32个输入，2个输出
#
#     def forward(self, x):
#         x = self.fc1(x)
#         x = self.relu(x)
#         x = self.fc2(x)
#         x = self.relu(x)
#         x = self.fc3(x)  # 输出层，不需要激活函数，如果是分类问题，可以使用softmax
#         return x
...
def federated_stream_artificial():
    ...
    for epoch in range(global_epochs):
        client_weights = {client_id: 1 for client_id in range(num_clients)}
        total_accuracy = 0.0
        local_models = []
        ...
        for client_id in range(num_clients):
            ...
            if ifdetection:
                # x[client_id] = x[client_id] + 1
                # print(f"在epoch{epoch}客户端{client_id}数据流发生漂移！")
                drift_points.append((epoch, client_id))  # 记录漂移点
                drift_weight_factor = drift_weight_factor + 5
                # client_weights.append(drift_weight_factor)  # 添加漂移权重
                # client_weights[client_id] = drift_weight_factor * 1 / x[client_id]
                client_weights[client_id] = drift_weight_factor
            # else:
            #     if client_weights[client_id] > 1:
            #         drift_weight_factor = drift_weight_factor - 1
            #         client_weights[client_id] = drift_weight_factor

            # client_weights.append(1.0)  # 正常权重

            # loss = train_local_model(local_model[client_id], client_data, local_epoch, initial_learning_rate)
        ...
        for global_param, local_param_list in zip(global_model.parameters(),
                                                  zip(*[local_model[client_id].parameters() for
                                                        local_model[client_id] in local_models])):
            # 将参数堆叠并加权平均
            weighted_sum = sum(weight * param for weight, param in zip(client_weights, local_param_list))
    ...
    # plt.plot(range(1, global_epochs + 1), loss_values, label='Training Loss')
    # plt.xlabel('Epoch')
    # plt.ylabel('Loss')
...
if __name__ == "__main__":
    ...
    plt.grid(True)
    plt.savefig(f"./new-results/{drift_Flie_name}_client_weight_average_global_accuracy.png")
    plt.show()
